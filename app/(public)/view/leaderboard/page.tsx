...
export default function PublicLeaderboardPage() {
  ...
  const topRated = useQuery(api.publicRestaurants.getTopRatedPublic, { limit: 10 });
  const topFood = useQuery(api.publicRestaurants.getCategoryLeadersPublic, {
    category: "food",
    limit: 5,
  });
  const topService = useQuery(api.publicRestaurants.getCategoryLeadersPublic, {
    category: "service",
    limit: 5,
  });
  const topExtras = useQuery(api.publicRestaurants.getCategoryLeadersPublic, {
    category: "extras",
    limit: 5,
  });
  const topAtmosphere = useQuery(api.publicRestaurants.getCategoryLeadersPublic, {
    category: "atmosphere",
    limit: 5,
  });
  const topBookers = useQuery(api.publicRatings.getTopBookersPublic, { limit: 10 });
        ...
        {view === "curries" ? (
          <>
            ...
            <section>
              ...
              {topRated === undefined ? (
                ...
                <EmptyCard message="No ratings yet." />
              ) : (
          ...
          <>
            ...
            {topBookers === undefined ? (
              ...
              <EmptyCard message="No bookers yet." />
            ) : (
              <>
                {/* Podium - Top 3 */}
                {topBookers.length >= 1 && (
